...
export async function getServerSideProps({params}) {
  ...
  const response = await fetch(`https://main.d6iszn1o7sirg.amplifyapp.com/api/blogArticle?BlogArticleId=${BlogArticleId}`);
  if (!response.ok) {
...
function BlogArticleLayout({ blogArt}) {
  //   const { userId } = auth();
      ...
      <section className="section  mt-20 section section4 mx-auto md:px-20 py-16 lg:w-4/6 md:w-5.25/6 ">
        <div className="container-section4 ">
          <div className="cat flex gap-6 pt-2 pb-2 category__postId">
            ...
            <Link
              href={`/Categories/${
                category == "OPINIONS" || category == "ANALYSIS" || category == "GEOPOL"
                  ? "NEWS"
                  : category
              }`}
              legacyBehavior
            ...
            <h2 className="article__text content ">
              <div className="h-full ">
                <div className="h-[80px] md:pl-50  inset-y-0 w-full ">
                  {blogArticle && (
                    <BlogArticleNavbar blogarticle={blogArticle} selectedChapter={selectedChapter}/>
                  )}
