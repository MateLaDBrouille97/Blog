...
import ChapterIdPage from "./chapters/[ChapterId]/page";
...
function BlogArticleLayout({ children }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (!blogArticle) {
          ...
          setCategory(fetchedBlogArticle?.category?.name);
        }
      } catch (error) {
    ...
    fetchData();
    fetchUser();
  }, [BlogArticleId, blogArticle?.chapters, router, blogArticle?.author]);
