...
function Post({ post }) {
  ...
  function convertAwsTimestampToDate(post) {
    const date = new Date(parseInt(post?.createdAt) * 1000); // Convert from seconds to milliseconds
        ...
        <div className="cat flex gap-6">
          ...
          <Link href="/" legacyBehavior>
            <a className="text-gray-600 hover:text-gray-800">
              {()=>convertAwsTimestampToDate(post) || "UnKnown"}
            </a>
          </Link>
        </div>
        <div className="title">
