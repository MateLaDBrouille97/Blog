...
function Category({ category, posts }) {
  ...
  const pageData = dataSort.slice(9).slice(skip, endIndex);
  ...
  return (
    <Format>
      ...
      <section className="section section__cat mx-auto md:px-2 py-8 w-3/4">
       ...
       { dataSort?.slice(0, 9) &&<div className="flex justify-center items-center">
          <Pagination page={1} perPage={12} itemCount={dataSort?.slice(0, 9)?.length?? 0} />
        </div>}
      </section>
    </Format>
  );
