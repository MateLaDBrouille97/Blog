...
import { subCategoryNews,subCategoryHacks,subCategoryProject } from "@/public/subCategoryName";
import ScrollBar from "@/components/_child/ScrollBar";
...
function Category({ category, posts }) {
  ...
  const postsData = () => {
    switch (category?.name) {
      ...
      case "NEWS":
        if (item.name === "news") {
          const blog3 = posts.filter((d) => {return d?.category.toLowerCase() === "news"});
          ...
          const blog4 = posts.filter((d) => {return d?.category.toLowerCase() === "opinions"});
  ...
  return (
    <Format>
      ...
      <section className="section section__cat mx-auto md:px-2 py-8 w-3/4">
        ...
        ) :category?.name == "USEFULHACKS"?(<div className="work__filters">
      ...
      </div>) :category?.name == "PROJECTS"?(<div className="work__filters">
