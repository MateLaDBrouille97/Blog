  ...
  return (
    <Format>
      <section className="container mx-auto md:px-2 py-16 w-1/2">
        ...
        <h1 className="font-bold text-4xl py-12 flex justify-center item-center text-center">Latest Posts</h1>
      {/* grid columns */}
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-14">
        {posts? posts.map((post, index) => <Post post={post} key={post.id} />):<><div className="flex flex-col justify-center item-center"><Error /></div></>}
      </div>
      </section>
    </Format>
  );
