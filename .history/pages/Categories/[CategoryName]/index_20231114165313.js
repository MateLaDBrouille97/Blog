  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const filteredArticles3 = blogArticle.filter((article) => {
        return article?.category?.name === "NEWS" |article?.category?.name === "ANALYSIS" |article?.category?.name === "OPINIONS" |article?.category?.name === "GEOPOL"
      ...
      switch (category?.name) {
        case "USEFULHACKS":
          if (item.name === "productivity") {
            ...
            const blog4 = posts.filter((d) => {
              if (d.isPublished) {
                return d?.subcategory?.name?.toLowerCase() === "research_tools";
              } else {
        ...
        case "NEWS":
          if (item.name === "news") {
            ...
            const blog4 = posts.filter((d) => {
              ...
              if (d.isPublished) {
                return d?.category?.name === "NEWS" ||
                "ANALYSIS" ||
                "OPINIONS" ||
                "GEOPOL";
              } else {
                return d?.category === "NEWS" ||
                "ANALYSIS" ||
                "OPINIONS" ||
                "GEOPOL";
              }
