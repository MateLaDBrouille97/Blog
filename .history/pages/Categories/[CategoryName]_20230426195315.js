...
export default function CategoryName() {
  ...
  return (
    <>
      ...
      >
        {cat ? (
          <Category category={cat} posts={posts} key={cat?.id} itemsPerPage={8}/>
        ) : (
          <Category
...
function Category({ category, posts,itemsPerPage }) {
  const router = useRouter();
        ...
        {posts ? (
          <div className=" posts__Col__older">
            {dataSort &&
              dataSort
                .slice(9 + 1)
                .map((post, index) => <PostCol2 post={post} key={post.id} />)}
          </div>
        ) : (
          <div className="flex flex-col justify-center item-center">
            {" "}
...
ReactDOM.render(
  <Category  itemsPerPage={8}/>,
  document.getElementById('posts__Col__older')
);
