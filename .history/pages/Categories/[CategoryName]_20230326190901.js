  ...
  useEffect(() => {
    const postsData = () => {
      switch (category?.name) {
        ...
        //   break;
        case "NEWS":
          if (subCategoryNews === "News") {
            const blog3 = posts.filter((d) => d?.category.toLowerCase() == subCategoryNews);
            ...
            setDataSort(data1);
            break;
          } else if(subCategoryNews === "Opinions") {
            const blog4 = posts.filter((d) => d?.category.toLowerCase() == subCategoryNews);
