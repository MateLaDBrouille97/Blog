...
function Category({ category, posts }) {
  ...
  const [item, setItem] = useState({ name: "News" });
  ...
  const postsData = () => {
    switch (category?.name) {
      ...
      //   break;
      case "NEWS":
        if (subCategoryNews?.name == "News") {
          const blog3 = posts.filter((d) => d?.category == "NEWS");
          ...
          setDataSort(data1);
          break;
        } else if(subCategoryNews?.name == "Opinions") {
          const blog4 = posts.filter((d) => d?.category == "OPINIONS");
    ...
    postsData();
  }, [posts, category,subCategoryNews]);
