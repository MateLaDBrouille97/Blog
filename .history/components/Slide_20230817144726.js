...
export default function Slide({ post }) {
  ...
  return (
    <div className="grid md:grid-cols-2 slide__container">
      <div className="image custom-image-wrapper7">
        <Link href={`/Posts/${post?.slug}`} legacyBehavior>
          <a>
            {!postImage ? (
              <Spinner />
            ) : (
              <Image src={postImage} alt="" width={600} height={800} className="postCol__img custom-image5" />
            )}
          </a>
        </Link>
      </div>
