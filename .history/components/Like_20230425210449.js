...
export default function Like({ id }) {
  ...
  useEffect(() => {
    async function fetchData() {
      // You can await here
      let theLike = post?.likeCount ||0;
       setLikesCount(theLike);
    }
    fetchData();
  ...
  useEffect(() => {
    const fetchBlog = async () => {
      await DataStore.query(BlogPost, (post) => post.id.eq(id)).then(
        (post) => setPost(post[0])
      );
    };
    fetchBlog();
...
const deleteLike = async () => {
  ...
  // Event Listeners
  const el = document.getElementById('likeBlock');
  if(el)
  {likeBtn.addEventListener("click", likeClick);}

  return (
    <div className="like_container">
      <div className="block" id="likeBlock"
      // onClick={likeClick}
      >
        <svg
