...
export default function RelatedBA({ post ,articles}) {
  ...
  useEffect(() => {
    ...
    const dataRelated = dataTotal?.filter(
      (post,value) => (post?.isPublished ? value.category === post?.category?.name : value.category === post?.category)
    );
