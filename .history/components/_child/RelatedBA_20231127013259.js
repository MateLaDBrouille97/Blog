...
export default function RelatedBA({ post, articles }) {
  ...
  useEffect(() => {
    const dataRelated = data?.concat(dataSort3 || []).filter(
      (value) => (post?.isPublished ? value?.category?.name === post?.category?.name : value?.category === post?.category)
    );


    console.log(dataRelated) 
   
    const dataRelated3 = dataRelated.sort((a, b) =>
