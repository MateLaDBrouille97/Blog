...
export default function RelatedBA({ post, articles }) {
  ...
  useEffect(() => {
    const dataRelated1 = dataSort3?.filter((value) =>{
      
        return value.category === post?.category?.name;
      
    }
    );

    const dataRelated2 = data?.filter((value) =>{
        return value.category === post?.category;
    }
    
    );
    const dataT =[...dataRelated1,...dataRelated2]
    const dataRelated3 = dataT.sort((a, b) =>
      b.createdAt.localeCompare(a.createdAt)
    );

    console.log(dataRelated3)
    setRelated(dataRelated3);
  }, [post, dataTotal, dataSort3, data]);
