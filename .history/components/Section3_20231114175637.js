...
export default function Section3() {
  ...
  useEffect(() => {
    const fetchDataSorted = async () => {
      ...
      const sortedData = dataUFH.sort((a, b) => b.createdAt.localeCompare(a.createdAt));
                ...
                {dataSort &&
                  dataSort.slice(0, 6).map((post, index) => (
                    (<SwiperSlide key={index}>
                     ...
                     {(post.isPublished? <PostLineBA post={post} key={post.id} />:<PostLine post={post} key={post.id} />)}
                    </SwiperSlide>)
                  ))}
              </Swiper>
            </div>
