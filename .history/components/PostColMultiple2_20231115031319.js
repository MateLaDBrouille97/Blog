        ...
        <div className="top">
          <div className="topLeft">
            <div className="topLeft__container">
              ...
              {(allposts[0]?.isPublished? <PostP7BA post={allposts[0]} key={allposts?.[0]?.id} />: <PostP7 post={allposts[0]} key={allposts?.[0]?.id} />)}
            </div>
          </div>
          <div className="topRight">
            <div className="topRight__container ">
              {allposts && allposts ? (
                allposts
                  .slice(1, 3)
                  .map((post, index) => {(post?.isPublished?<PostP8BA post={post} key={post.id} />:<PostP8 post={post} key={post.id} />)})
              ) : (
                <></>
              )}
            </div>
        ...
        <div className="bottom">
          <div className="bottom__container">
            <div className="bottom-left">
                ...
                allposts
                  .slice(3, 5)
                  .map((post, index) => {(post?.isPublished?<PostCol32BA post={post} key={post.id} />:<PostCol32 post={post} key={post.id} />)})
              ) : (
                <></>
              )}
              </div>  
