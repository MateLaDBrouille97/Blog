...
const [postsfiltered, setFiltered] = useState([]);
  ...
  useEffect(() => {
    const fetchDataSorted = async () => {
      const blogArt = await getArticles();
  ...
  useEffect(() => {
    const postsD = () => {
      ...
      try {
        switch (categoryName) {
          case "USEFULHACKS":
            const postsUFH = [...postsUfh];
            setPosts(postsUFH);
            break;
          case "PROJECTS":
            const postsP = [...postsProj];
            setPosts(postsP);
            break;
          case "NEWS":
            const blogmerge = [
            ...
            break;
          case "POSTS":
            const postsData = [...blogArticle];
            setPosts(postsData);
            break;
          case "TOOLS":
            const postsT = [...postsTools];
            setPosts(postsT);
  ...
  return (
    <>
      <div
        id={
          params.CategoryName == "USEFULHACKS"
            ? "usefulHacks"
            : params.CategoryName == "PROJECTS"
            ? "projects"
            : "news"
        }
      >
